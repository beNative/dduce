  ...
  begin
    ...
    if Result then
      ...
      begin
        ...
        s := FTree.Text[FNode, FColumn];
        FTree.GetTextInfo(FNode, FColumn, TWinControlAccess(EditControl).Font, FTextBounds, s);
        PrepareEditControl;
        SetEditText(s);
      end;
  end;
  ...
  if not FStopping then
    with FTree do
      if (toAutoAcceptEditChange in TreeOptions.StringOptions) and Modified then
        Text[FNode, FColumn] := GetEditText;
...
begin
  Result := Copy(TMemo(FEdit).Text, 1, Length(TMemo(FEdit).Text) - 2);
end;
